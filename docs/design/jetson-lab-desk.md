...
1. 저장된 SSH 비밀번호를 **실제 명령과 같은 exec 채널의 stdin**에 넣는다:
   `sudo -k -S -p '' loginctl enable-linger <user>`. `-k`로 캐시된 timestamp를
   무효화해서 "지금 이 비밀번호가 맞는가"만 판정하고, `-p ''`로 프롬프트 문자열을
   없애 stderr 파싱을 단순하게 만든다.
...
**검증과 실행을 나누면 안 된다.** 처음 구현은 `sudo -k -S -p '' -v`로 비밀번호를
확인한 뒤, 다음 exec에서 `sudo -n`으로 실제 명령을 돌렸다. 이 방식은 어떤 비밀번호를
넣어도 실패한다 — sudo의 timestamp는 tty에 묶이는데(`timestamp_type=tty`가 기본)
비대화형 SSH exec에는 tty가 없어 채널마다 다른 기록을 쓴다. 실물에서 확인했다:
`-v`는 0을 반환하고, **바로 다음 exec가 "sudo: a password is required"를 낸다.**
비밀번호는 그것이 승인하는 명령과 같은 exec에 실려야 한다.
...
### 앱이 죽었을 때 — 그리고 장비가 죽었을 때

user unit이므로 Jetson 쪽 서버는 그대로 떠 있다. 앱을 다시 켜면 탐색 후
`is-active`로 "이미 돌고 있음"을 알아내고 그 상태로 복귀한다. PID 파일 방식보다
회수가 깔끔하다.

반대 방향도 같은 규칙을 따라야 한다. **화면 상태는 앱이 한 일의 기억이 아니라
장비의 대답이어야 한다.** 실물에서 Jetson을 재부팅했더니 카드가 계속 "running"을
가리키고 `시작` 버튼이 비활성인 채로 남아, 서버를 다시 띄울 방법이 화면에
없었다. 그래서 설치된 장비마다 `is-active` + `/api/health`를 주기적으로 물어
(10초) 그 답으로 카드를 맞추고, 서버가 살아 있는데 터널이 없으면 터널도 그때
연다 — 터널이 `시작` 버튼의 부수 효과로만 열리면, 이미 떠 있는 서버에서는
프리뷰가 영원히 "터널 없음"이 된다.
...
- **응답하지 않는 카메라** — USB 링크가 무너지는 중인 카메라는 노드를 남긴 채
  open/read에서 무한정 멈춘다. 실물에서 이것 하나가 장치 열거를 붙잡아 서버
  전체를 잠갔다(모든 요청이 "camera is busy"). 그래서 열거 프로브에 8초 제한을
  두고(`PROBE_TIMEOUT_S`), 못 지킨 장치는 `probeError: "timeout"`으로 보고한다.
  다만 **V4L2의 블로킹 read는 취소할 수 없다** — 스트리밍 중에 카메라가 죽으면
  그 캡처는 프로세스가 놓지 못하므로 서버 재시작이 유일한 회수 방법이다. 이 한계는
  감추지 말고 그대로 보고한다.
- **열거는 프리뷰를 뺏지 않는다** — 열거는 카메라를 독점으로 열어야 하고 이
  장비에서 3대에 약 18초가 걸린다. 프리뷰가 떠 있는 동안의 일상적인 조회는 마지막
  목록을 그대로 돌려주고(`X-Devices-Cached: 1`), 다시 프로브하는 것은 사용자가
  "다시 감지"를 눌렀을 때(`?refresh=1`)뿐이다. 창 포커스 같은 사건이 카메라를
  뺏어가서는 안 되므로 renderer의 focus 재조회도 끈다.
